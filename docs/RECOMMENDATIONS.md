...
- **JWT_SECRET** — в коде есть fallback `'dev-secret-change-in-production'`. В проде обязательно задавать свой длинный случайный `JWT_SECRET` в `.env`. **При старте в prod** бэкенд пишет в лог `[SECURITY] В проде задайте свой длинный случайный JWT_SECRET`, если секрет не задан или равен дефолту.
